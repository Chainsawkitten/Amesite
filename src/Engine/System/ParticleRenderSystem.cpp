...
void ParticleRenderSystem::Render(Scene & scene, Entity* camera, const glm::vec2& screenSize) {
    if (Particle().ParticleCount() > 0) {
        ...
        glm::mat4 view = camera->GetComponent<Component::Transform>()->GetWorldOrientation() * glm::translate(glm::mat4(), -camera->GetComponent<Component::Transform>()->position);
        glm::vec3 up(glm::inverse(camera->GetComponent<Component::Transform>()->GetWorldOrientation())* glm::vec4(0, 1, 0, 1));
