        ...
        if (healthComponent->health < 0.01f && healthComponent->removeOnLowHealth) {
            if (healthComponent->health < 0.01f && healthComponent->removeOnLowHealth) {
                ...
                if (explodeComp != nullptr)
                    GameEntityCreator().CreateExplosion(healthComponent->entity->GetComponent<Component::Transform>()->GetWorldPosition()+explodeComp->offset, explodeComp->lifeTime, explodeComp->size, explodeComp->particleTextureIndex); // Create Explosion
